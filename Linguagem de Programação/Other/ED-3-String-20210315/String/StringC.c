...
int append(String *s1, String *s2)
{
    ...
    // Copia dados de 's2' para o final de 's1'
    int i;
...
String *sub_str(String *s1, int ini, int fim, int *status)
{
    ...
    if (s1 == NULL) {
        stat = ERR_VOID_STR;
    } else if (s1->str_txt == NULL) {
        stat = ERR_INVALID_STR;
    } else if (fim < ini || fim >= s1->str_len) {
        stat = ERR_INVALID_IDX;
    }
    ...
    int result_len = fim - ini + 1;
    int num_blocks = (result_len/STR_BLOCK) + 1;

    // Criar string vazia com o tamanho determinado
    String *s = create_raw_str(num_blocks * STR_BLOCK);

    // Copia dados da substring
    int i;
    for (i = 0; i < result_len; i++) {
        s->str_txt[i] = s1->str_txt[i + ini];
    }
    s->str_txt[result_len] = '\0';
...
char *get_txt(String *s1)
{
    ...
    if (s1->str_txt == NULL) return NULL;
    return s1->str_txt;
};
